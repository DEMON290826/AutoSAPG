...
def _find_auto_gpt_root() -> Path:
    """
    Search order:
    1. Same folder as runner_story.py      <- user puts Auto_GPT.py here (most common)
    2. Parent folder                       <- packaged: resources/
    3. Next to python.exe                  <- packaged alternative
    4. Dev: 3 levels up (Auto_All/)        <- dev mode
    """
    candidates = [
        Path(__file__).resolve().parent,               # same dir as runner_story.py
    ...
    print(json.dumps({"type": "error",
        "message": f"Auto_GPT.py not found! Checked: {checked}. "
                   f"Please put Auto_GPT.py in the same folder as runner_story.py"}))
    return candidates[0]
